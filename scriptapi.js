...
const getAll = async () => {
    try {
        let res = await axios.get("http://localhost:4444/calendario")
    ...
    if (e.target === $form) {
        ...
        if (!e.target.id.value) {
            try {
                ...
                let res = await axios("http://localhost:4444/calendario", options)
                let json = await res.data;
            ...
            try {
                ...
                let res = await axios(`http://localhost:4444/calendario/${e.target.id.value}`, options)
                let json = await res.data;
    ...
    if (e.target.matches(".delete")){
        ...
        if (confirmacion) {
            try {
                ...
                let res = await axios(`http://localhost:4444/calendario/${e.target.dataset.id}`, options)
                let json = await res.data;
